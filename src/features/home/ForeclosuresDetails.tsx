...
import { foreclosureProperties } from '@/constants/PropertiesDetailsData';
...
const ForeclosuresDetails = () => {
	const [deposit, setDeposit] = useState<number>(10);
	const [loanTerm, setLoanTerm] = useState<number>(5);
	const [interestRate, setInterestRate] = useState<number>(10);
	const { foreclosureId } = useParams<{ foreclosureId: string }>();
	const property = foreclosureProperties.find((item) => item.id === foreclosureId);
	...
	return (
		<div>
			<PropertiesDetailsCard
				...
				icons={icons}
				breadCrumb="FORECLOSURES > HOUSES > "
				type="house"
				showAvailable={true}
...
export default ForeclosuresDetails;
